...
/**
 * ChEMBL Search via Valyu API
 * Full-text search across ChEMBL with semantic search capabilities
 */
...
/**
 * Search ChEMBL via Valyu API
 */
async function searchChEMBL(query, maxResults = 10) {
...
(async () => {
  ...
  if (command === 'setup') {
    ...
    if (!query) {
      ...
      result = await searchChEMBL(query, maxResults);
    }
  }
