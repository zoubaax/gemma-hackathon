...
/**
 * Drug Labels Search via Valyu API
 * Full-text search across Drug Labels with semantic search capabilities
 */
...
/**
 * Search Drug Labels via Valyu API
 */
async function searchDrug_Labels(query, maxResults = 10) {
...
(async () => {
  ...
  if (command === 'setup') {
    ...
    if (!query) {
      ...
      result = await searchDrug_Labels(query, maxResults);
    }
  }
