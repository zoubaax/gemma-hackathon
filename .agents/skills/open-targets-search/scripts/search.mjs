...
/**
 * Open Targets Search via Valyu API
 * Full-text search across Open Targets with semantic search capabilities
 */
...
/**
 * Search Open Targets via Valyu API
 */
async function searchOpen_Targets(query, maxResults = 10) {
...
(async () => {
  ...
  if (command === 'setup') {
    ...
    if (!query) {
      ...
      result = await searchOpen_Targets(query, maxResults);
    }
  }
