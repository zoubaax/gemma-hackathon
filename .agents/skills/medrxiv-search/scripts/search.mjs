...
/**
 * medRxiv Search via Valyu API
 * Full-text search across medRxiv with semantic search capabilities
 */
...
/**
 * Search medRxiv via Valyu API
 */
async function searchmedRxiv(query, maxResults = 10) {
...
(async () => {
  ...
  if (command === 'setup') {
    ...
    if (!query) {
      ...
      result = await searchmedRxiv(query, maxResults);
    }
  }
